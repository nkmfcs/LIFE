      ...
      <CardHead
        ...
        right={
          <span className="t-small" style={{ color: "var(--ink-mute)" }}>
            {thisWeekCount}/{weekGoal} на этой неделе
          </span>
        }
      />
      ...
      {showForm && (
        ...
        >
          <div className="row" style={{ gap: 8 }}>
            <select style={{ ...inp, flex: 1 }} value={type} onChange={(e) => setType(e.target.value)}>
              {TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
            <select style={{ ...inp, flex: 1 }} value={intensity} onChange={(e) => setIntensity(e.target.value)}>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <p className="t-micro" style={{ color: "var(--ink-soft)", marginTop: 2 }}>
              {t.date} · {t.intensity}{t.notes ? ` · ${t.notes}` : ""}
            </p>
          </div>
